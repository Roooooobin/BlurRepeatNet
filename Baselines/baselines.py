...
class Pop:
    def __init__(self, top_n=100, item_key="ItemId"):
        self.top_n = top_n
    ...
    def fit(self, data):
        ...
        self.pop_list = self.pop_list / (self.pop_list + 1)  # 将值映射到[0, 1)之间
        self.pop_list.sort_values(ascending=False, inplace=True)
        self.pop_list = self.pop_list.head(self.top_n)

    def predict(self, session_id, input_item_id, predict_for_item_ids):
...
class SessionPop:
    ...
    def predict(self, session_id, input_item_id, predict_for_item_ids):
        if self.pre_session_id != session_id:
            self.pre_session_id = session_id
            self.pers = {}
        v = self.pers.get(input_item_id, 0)
        self.pers[input_item_id] = v + 1
        preds = np.zeros(len(predict_for_item_ids))
        mask = np.in1d(predict_for_item_ids, self.pop_list.index)
        series = pd.Series(self.pers)
        preds[mask] = self.pop_list[predict_for_item_ids[mask]]
        mask = np.in1d(predict_for_item_ids, series.index)
        preds[mask] += series[predict_for_item_ids[mask]]
...
class ItemKNN:
    ...
    def __init__(self, n_sims=100, lmbda=20, alpha=0.5,
    ...
    def fit(self, data):
        ...
        count_items = len(item_ids)
        data = pd.merge(data, pd.DataFrame({self.item_key: item_ids, "item_idx": np.arange(len(item_ids))}),
                        on=self.item_key, how="inner")
        session_ids = data[self.session_key].unique()
        data = pd.merge(data, pd.DataFrame({self.session_key: session_ids, "session_idx": np.arange(len(session_ids))}),
        ...
        index_by_sessions = data.sort_values(["session_idx", self.time_key]).index.values
        supp = data.groupby("item_idx").size()
        item_offsets = np.zeros(len(supp) + 1, dtype=np.int32)
        item_offsets[1:] = supp.cumsum()
        index_by_items = data.sort_values(["item_idx", self.time_key]).index.values
        for i in range(count_items):
            iarray = np.zeros(count_items)
            ...
            for e in index_by_items[start:end]:
                ...
                iarray[data.item_idx.values[user_events]] += 1
            iarray[i] = 0
            norm = np.power((supp[i] + self.lmbda), self.alpha) * np.power((supp.values + self.lmbda), (1.0 - self.alpha))
            norm[norm == 0] = 1
            iarray = iarray / norm
            indices = np.argsort(iarray)[-1:-1 - self.n_sims:-1]
            self.similarities[item_ids[i]] = pd.Series(data=iarray[indices], index=item_ids[indices])

    def predict(self, session_id, input_item_id, predict_for_item_ids):
        preds = np.zeros(len(predict_for_item_ids))
        sim_list = self.similarities[input_item_id]
...
class BPR:
    def __init__(self, n_factors=100, n_iterations=10, learning_rate=0.01, lambda_session=0.0,
                 lambda_item=0.0, sigma=0.05, init_normal=False, session_key="SessionId", item_key="ItemId"):
    ...
    def init(self, data):
        self.U = np.random.rand(self.n_sessions, self.n_factors) * 2 * self.sigma - self.sigma if not self.init_normal \
            else np.random.randn(self.n_sessions, self.n_factors) * self.sigma
        self.I = np.random.rand(self.n_items, self.n_factors) * 2 * self.sigma - self.sigma if not self.init_normal \
            else np.random.randn(self.n_items, self.n_factors) * self.sigma
    ...
    def fit(self, data):
        ...
        self.itemidmap = pd.Series(data=np.arange(self.n_items), index=item_ids)
        sessionids = data[self.session_key].unique()
        self.n_sessions = len(sessionids)
        data = pd.merge(data, pd.DataFrame({self.item_key: item_ids, "ItemIdx": np.arange(self.n_items)}),
                        on=self.item_key, how="inner")
    ...
    def predict(self, session_id, input_item_id, predict_for_item_ids):
        item_idx = self.itemidmap[input_item_id]
        if self.current_session is None or self.current_session != session_id:
            self.current_session = session_id
            self.session = [item_idx]
        else:
            self.session.append(item_idx)
...
class GRU4Rec:
    ...
    def __init__(self, loss='bpr-max', final_act='linear', hidden_act='tanh', layers=[100],
                 n_epochs=10, batch_size=32, dropout_p_hidden=0.0, dropout_p_embed=0.0, learning_rate=0.1, momentum=0.0, lmbd=0.0,
                 embedding=0, n_sample=2048, sample_alpha=0.75, smoothing=0.0, constrained_embedding=False,
                 adapt='adagrad', adapt_params=[], grad_cap=0.0, bpreg=1.0,
                 sigma=0.0, init_as_normal=False, train_random_order=False, time_sort=True,
                 session_key='SessionId', item_key='ItemId', time_key='Time'):
        self.layers = layers
    ...
    def cross_entropy(self, yhat, M):
        if self.smoothing:
            ...
            return T.cast(T.mean((1.0 - (n_out / (n_out - 1)) * self.smoothing) * (-T.log(gpu_diag_wide(yhat) + 1e-24)) + (
    ...
    def cross_entropy_logits(self, yhat, M):
        if self.smoothing:
            ...
            return T.cast(T.mean(
                (1.0 - (n_out / (n_out - 1)) * self.smoothing) * gpu_diag_wide(yhat) + (self.smoothing / (n_out - 1)) * T.sum(
                    yhat, axis=1)), theano.config.floatX)
        else:
    ...
    def bpr_max(self, yhat, M):
        ...
        return T.cast(T.mean(-T.log(T.sum(T.nnet.sigmoid(gpu_diag_wide(yhat).dimshuffle((0, 'x')) - yhat) * softmax_scores,
                                          axis=1) + 1e-24) + self.bpreg * T.sum((yhat ** 2) * softmax_scores, axis=1)),
    ...
    def init(self, data):
        ...
        for i in range(len(self.layers)):
            ...
            self.Bh.append(theano.shared(value=np.zeros((self.layers[i] * 3,), dtype=theano.config.floatX), borrow=True,
                                         name='Bh{}'.format(i)))
            self.H.append(
                theano.shared(value=np.zeros((self.batch_size, self.layers[i]), dtype=theano.config.floatX), borrow=True,
                              name='H{}'.format(i)))
    ...
    def adam(self, param, grad, updates, sample_idx=None, epsilon=1e-6):
        ...
        if sample_idx is None:
            ...
            updates[meang] = T.inc_subtensor(T.set_subtensor(meang_s, meang_s * v1)[sample_idx],
    ...
    def RMSprop(self, cost, params, full_params, sampled_params, sidxs, epsilon=1e-6):
        ...
        if self.grad_cap > 0:
            norm = T.cast(T.sqrt(
                T.sum([T.sum([T.sum(g ** 2) for g in g_list]) for g_list in grads]) + T.sum([T.sum(g ** 2) for g in sgrads])),
                          theano.config.floatX)
    ...
    def fit(self, data, sample_store=10000000):
        ...
        data = pd.merge(data, pd.DataFrame({self.item_key: itemids, 'ItemIdx': self.itemidmap[itemids].values}), on=self.item_key,
                        how='inner')
        ...
        base_order = np.argsort(data.groupby(self.session_key)[self.time_key].min().values) if self.time_sort else np.arange(
